 ...
 */
export const SourceItemList: React.FC<{}> = (): JSX.Element | null => {
    ...
    const [selectedSourceItem, setSelectedSourceItem] = useRecoilState(
        selectedSourceItemState
    );

    const selectedSourceItemList = sourceItemList.find(
        (sourceItem) => sourceItem.sourceId === selectedSourceId
    );
    ...
    useEffect(() => {
        ...
        if (
            ...
            const sourceUrl = sourceList.find(
                (source) => source.id === selectedSourceId
            )?.url;
            if (sourceUrl) {
                // next, load and parse the source stream
                parse(sourceUrl).then((result) => {
    ...
    return selectedSourceItemList ? (
        <ul>
            {selectedSourceItemList.items.map((sourceItem) => (
                ...
                >
                    <div className="source-namet">{sourceItem.title}</div>
                </li>
            ))}
        </ul>
    ) : null;
